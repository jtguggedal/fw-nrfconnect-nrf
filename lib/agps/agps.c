...
LOG_MODULE_REGISTER(supl, CONFIG_AGPS_LOG_LEVEL);
			    ...
			    void *user_data)
{
	...
	LOG_DBG("Injected AGPS data, flags: %d, size: %d", type, agps_size);
	...
	do {
		...
		if (err) {
			...
			LOG_ERR("Failed to resolve IPv4 hostname %s, errno: %d",
				CONFIG_AGPS_SUPL_HOST_NAME, errno);
...
static int init_supl(int *socket)
{
	...
	if (socket) {
		LOG_DBG("Using user-provided socket, fd %d", gnss_fd);
...
static int supl_start(const struct gps_agps_request request)
{
	...
	nrf_gnss_agps_data_frame_t req = {
		...
		.data_flags =
			(request.utc ? BIT(NRF_GNSS_AGPS_GPS_UTC_REQUEST) : 0) |
			(request.klobuchar ? BIT(NRF_GNSS_AGPS_KLOBUCHAR_REQUEST) : 0) |
			(request.nequick ? BIT(NRF_GNSS_AGPS_NEQUICK_REQUEST) : 0) |
			(request.system_time_tow ? BIT(NRF_GNSS_AGPS_SYS_TIME_AND_SV_TOW_REQUEST) : 0) |
			(request.position ? BIT(NRF_GNSS_AGPS_POSITION_REQUEST) : 0) |
			(request.integrity ? BIT(NRF_GNSS_AGPS_INTEGRITY_REQUEST) : 0),
	};
	...
	if (err) {
		...
		LOG_INF("SUPL session finished succesfully");
	}
...
#endif	/* CONFIG_AGPS_SRC_NRF_CLOUD */
