...
struct dps_reg_status {
	...
	uint32_t retry;
	char reg_id[CONFIG_AZURE_IOT_HUB_DEVICE_ID_MAX_LEN];
...
static struct dps_reg_status dps_reg_status = {
	.reg_id = CONFIG_AZURE_IOT_HUB_DPS_REG_ID,
};
static struct mqtt_client *client;
static char dps_topic_reg_pub[sizeof(DPS_TOPIC_REG_PUB) + sizeof(dps_reg_status.reg_id)];
...
static int dps_settings_init(void)
{
	...
	if (err) {
		LOG_ERR("Failed to initialize settings subsystem, eror: %d",
			err);
		return err;
	}
...
static void dps_reg_status_work_fn(struct k_work *work)
{
	...
	if ((++counter <= DPS_REG_STATUS_UPDATE_MAX_RETRIES) &&
...
int dps_start(void)
{
	...
	// TODO: Call callback when registered
...
{
	if (strlen(dps_reg_status.reg_id) == 0) {
		LOG_INF("No DPS registration ID provided, using default ID");
		return NULL;
	}

	return dps_reg_status.reg_id;
}
