...
/* Counter for received GNSS module messagess. */
static uint32_t gnss_module_event_count;
/* Number of expected GNSS module messagess. */
static uint32_t expected_gnss_module_event_count;
/* Array for expected GNSS module messagess. */
...
/* Test whether sending a APP_MSG_DATA_GET event to the GNSS module generates
 * the GNSS_MSG_ACTIVE event, when the GNSS module is in the running state.
 */
void test_gnss_start(void)
{
	...
	/* Set callback to validate GNSS module messagess. */
	__wrap__event_submit_Stub(&validate_gnss_evt);

	/* Set expected GNSS module messagess. */
	expected_gnss_module_event_count = 1;
	expected_gnss_module_events[0].type = GNSS_MSG_ACTIVE;
	...
	app_module_event->type = APP_MSG_DATA_GET;
...
void test_gnss_fix(void)
{
	...
	expected_gnss_module_events[0].type = GNSS_MSG_ACTIVE;
	expected_gnss_module_events[1].type = GNSS_MSG_INACTIVE;
	expected_gnss_module_events[2].type = GNSS_MSG_DATA_READY;
	...
	gnss_module_gnss_evt_handler(NRF_MODEM_GNSS_MSG_PVT);

	gnss_module_gnss_evt_handler(NRF_MODEM_GNSS_MSG_NMEA);
}
...
void test_agps_request(void)
{
	...
	expected_gnss_module_events[0].type = GNSS_MSG_ACTIVE;
	expected_gnss_module_events[1].type = GNSS_MSG_AGPS_NEEDED;
	...
	gnss_module_gnss_evt_handler(NRF_MODEM_GNSS_MSG_AGPS_REQ);
}
...
void test_msgq_full(void)
{
	...
	expected_gnss_module_event_count = 2;
	expected_gnss_module_events[0].type = GNSS_MSG_ACTIVE;
	expected_gnss_module_events[1].type = GNSS_MSG_ERROR_CODE;
	...
	gnss_module_gnss_evt_handler(NRF_MODEM_GNSS_MSG_BLOCKED);
	gnss_module_gnss_evt_handler(NRF_MODEM_GNSS_MSG_UNBLOCKED);
	gnss_module_gnss_evt_handler(NRF_MODEM_GNSS_MSG_BLOCKED);
	gnss_module_gnss_evt_handler(NRF_MODEM_GNSS_MSG_UNBLOCKED);
	gnss_module_gnss_evt_handler(NRF_MODEM_GNSS_MSG_BLOCKED);
	gnss_module_gnss_evt_handler(NRF_MODEM_GNSS_MSG_UNBLOCKED);
