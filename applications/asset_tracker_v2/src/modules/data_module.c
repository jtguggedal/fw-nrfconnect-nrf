...
static void data_agps_request_encode(void)
{
	...
	switch (err) {
	case 0:
		LOG_DBG("AGPS request encoded successfully");
		data_send(DATA_EVT_AGPS_REQUEST_DATA_SEND, AGPS_REQUEST, &codec);
		break;
	case -ENOTSUP:
		/* AGPS request encoding not supported */
		break;
	case -ENODATA:
		LOG_DBG("No AGPS request data to encode, error: %d", err);
		break;
	default:
		LOG_ERR("Error encoding AGPS request: %d", err);
		SEND_ERROR(data, DATA_EVT_ERROR, err);
		return;
	}
}
